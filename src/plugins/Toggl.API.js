...
export default class TogglAPI {
	...
	static async #get(path, options = null) {
		try {
			const baseUrl = options?.isReportsAPI
					? config.baseUrlReportsAPI
					: config.baseUrl,
			...
			if (!authOptions) AuthorizationHeader = this.#APITokenString
			else
	...
	static async #handleAPIResponse(result) {
		if (result.ok) return await result.json()
			...
			await result
				.json() // Check if it is possible to get an error object.
				.then(errorObj => {
					throw { ...errorObj, status: result.status }
				})
				.catch(e => {
