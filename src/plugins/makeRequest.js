...
export default async (path, options) => {
	try {
		...
		if (requiresAuth) {
			...
			if (!authToken)
				throw new Error(
					'The route requires auth, but no authToken is found in the user_metadata!'
				)
			...
			switch (provider.name) {
				...
				case 'togglReports':
					if (options?.params?.email && options?.params?.password)
						fetchOptions.headers.Authorization = `Basic ${btoa(
							`${options.params.email}:${options.params.password}`
						)}`
					break
			}
		...
		if (!res.ok)
			throw new Error(
				'Error with the request. Response is: ' + (await res.text())
			)
...
const getProviderData = rawProviderOption => {
	if (!rawProviderOption) throw new Error('Please provide a provider.')

	let providerData =
		typeof rawProviderOption === 'string'
			? { name: rawProviderOption }
			: rawProviderOption
	...
	if (!providerDataFromConfig)
		throw new Error(
			`The given provider "${
				providerData.name
			}" is not included in allowed ones: "${Object.keys(
				_config.providers
			).join('", "')}".`
		)
...
const buildUrl = (provider, path) => {
	...
	if (provider.name === 'papierkram') {
		...
		else baseUrl = baseUrl.replace('{{subdomain}}', subdomain)
	}
