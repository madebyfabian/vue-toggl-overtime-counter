<template>
  <label class="input-field" :variant="variant">
    <span v-if="valuePrepend" v-text="valuePrepend" class="input-field__additional-value" />

    <input
      :value="value" 
      :type="computedType"
      :required="required"
      :placeholder="computedPlaceholder"
      :autocomplete="autocomplete"
      :spellcheck="spellcheck"
      :disable-password-managers="disablePasswordManagers"
      :value-append="valueAppend"
      :value-prepend="valuePrepend"
      @input="$emit('input', $event.target.value)"
    />

    <span v-if="valueAppend" v-text="valueAppend" class="input-field__additional-value" />
  </label>
</template>

<script>
  export default {
    name: 'InputField',

    props: {
      value: {
        type: String
      },
      type: {
        type: String, default: 'text'
      },
      placeholder: {
        type: String
      },
      required: {
        type: Boolean, default: true
      },
      autocomplete: {
        type: String, default: 'on'
      },
      spellcheck: {
        type: Boolean, default: true
      },

      valueAppend: {
        type: String, default: ''
      },
      valuePrepend: {
        type: String, default: ''
      },
      disablePasswordManagers: {
        type: Boolean, default: false
      },
      variant: {
        type: String, default: 'default'
      }
    },

    computed: {
      computedType() {
        if (this.type === 'password' && this.disablePasswordManagers)
          return 'password--password-mask'

        return this.type   
      },

      computedPlaceholder() {
        if (this.placeholder)
          return this.placeholder

        switch (this.type) {
          case 'email': 
            return 'E-Mail-Adresse'

          case 'password': case 'password--password-mask':
            return 'Passwort'

          case 'name':
            return 'Dein Name'
        
          default:
            return ''
        }
      }
    }
  }
</script>

<style lang="scss" scoped>
  @font-face {
    font-family: 'password-mask';
    src: url(data:font/woff;base64,) format("woff")
  }
  
  .input-field {
    position: relative;
    display: flex;
    height: 56px;
    background-color: #2F3437;
    border-radius: 8px;
    margin-bottom: 24px;

    &[variant=bottomLine] {
      background-color: transparent;
      border-radius: 0px;

      input {
        padding: 0;
        border-radius: 0;
        transition: border-color 150ms ease;
        border-bottom: 2px solid #454B4E;

        &:focus {
          box-shadow: none;
          border-color: rgba(#2EAADC, .5);
        }
      }
    }

    input {
      height: 100%;
      width: 100%;
      background: transparent;
      padding: 0 24px;
      border: none;
      outline: none;
      appearance: none;
      color: #fff;
      border-radius: 8px;
      display: block;
      min-width: 0; // Firefox Bugfix

      &[disable-password-managers=true] {
        background: none!important;
        background-image: none!important;
      }

      // Hack to disable password managers saving this form as an account, which we don't want!
      &[type=password--password-mask]:not(:placeholder-shown) {
        font-family: 'password-mask';
      }

      
      
      &::placeholder {
        color: rgba(#fff, .25);
      }
    }

    &__additional-value {
      height: 40px;
      background: #363B3E;
      color: rgba(#fff, .25);
      padding: 8px;
      line-height: 24px;
      display: block;
      margin: 8px;
      border-radius: 8px;
      user-select: none;
    }
  }
</style>